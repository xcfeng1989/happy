...
public class SumDeptSalary extends Configured implements Tool {


    @Override
    public int run(String[] args) throws Exception {
        // 实例化作业对象，设置作业名称、Mapper和Reduce类
        Job job = new Job(getConf(), "SumDeptSalary");
        job.setJobName("SumDeptSalary");
        job.setJarByClass(SumDeptSalary.class);
        job.setMapperClass(MapClass.class);
        job.setReducerClass(MapClass.Reduce.class);
        // 设置输入格式类
...
//        FileInputFormat.addInputPath(job, new Path(otherArgs[1]));
    ...
    public static void main(String[] args) throws Exception {
        int res = ToolRunner.run(new Configuration(), new SumDeptSalary(), args);
        System.exit(res);
    }
}
